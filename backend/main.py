...
def chat():
    ...
    prompt = f"""Act as a kind and helpful mental health assistant named LumoraAI.
...
to encourage the user to share more about their feelings."""
    ...
    # ✅ Always send IST time (Indian Standard Time)
    IST = timezone(timedelta(hours=5, minutes=30))
    ist_timestamp = datetime.now(IST).strftime("%Y-%m-%dT%H:%M:%S%z")
    return jsonify({
